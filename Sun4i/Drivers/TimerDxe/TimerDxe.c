...
{
  // Disable interrupts
  MmioAnd32(SUNXI_TIMER_BASE + SUNXI_TIMER_IRQEN, ~(1 << INTERRUPT_TIMER));
  ...
  )
{
  ...
  if (TimerPeriod != 0) {
    TimerTicks = MultU64x32(TimerPeriod, 24000000);
  ...
  )
{
  ...
  if (Sun4iTimerGetIsInterruptActive()) {
    if (mTimerNotifyFunction) {
      mTimerNotifyFunction (mTimerPeriod * mElapsedPeriod);
    }

    Sun4iTimerClearInterruptStatus();
  ...
  )
{
  ...
  Status = gInterrupt->RegisterInterruptSource(gInterrupt, TIMER_IRQ_VECTOR, TimerInterruptHandler);
  ASSERT_EFI_ERROR(Status);
  ...
  MmioOr32(SUNXI_TIMER_BASE + SUNXI_TIMER_IRQEN, (1 << INTERRUPT_TIMER));
